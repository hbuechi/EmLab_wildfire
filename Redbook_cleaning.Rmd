...
Month <- format(calfire_data$Start, "%m")
calfire_data <- dplyr::mutate(calfire_data, Month = Month)

# concatenate into a fire name / year column AND fire name / year / month column

calfire_conc <- calfire_data %>% 
  unite(Fire_year, FIRE_NAME, Year, remove = F) %>% 
  unite(Fire_year_month, FIRE_NAME, Year, Month, remove = F)
