...
redbook_1979 <- read_csv("CDF_fire_damage_1984-2017_V2 - 1979.csv")
redbook_1980 <- read_csv("CDF_fire_damage_1984-2017_V2 - 1980.csv")
redbook_1981 <- read_csv("CDF_fire_damage_1984-2017_V2 - 1981.csv")
redbook_1982 <- read_csv("CDF_fire_damage_1984-2017_V2 - 1982.csv")
redbook_1983 <- read_csv("CDF_fire_damage_1984-2017_V2 - 1983.csv")
...
# this is much easier to just do during the join in Arc because I can view the attribute table after the join and the common fires are highlighted, then I export that table and can play way more with it in R.
...
# classify Start and Contained columns as dates - my issue earlier in the code was that I wasn't specifying the format so R turned all of the dates to NA)

calfire_data$Start <- as.Date(calfire_data$Start, "%m/%d/%Y")
calfire_data$Contained <- as.Date(calfire_data$Contained, "%m/%d/%Y")

# new column with numeric month

Start_month <- format(calfire_data$Start, "%m")
calfire_data <- mutate(calfire_data, Start_month = Start_month)
calfire_data$Start_month <- as.integer(calfire_data$Start_month) # get rid of leading 0's
calfire_data <- calfire_data %>% 
  dplyr::rename(Fire_year=Year)

# concatenate into a fire name / year column AND fire name / year / month column

calfire_concat <- calfire_data %>% 
  unite(Firename_year, FIRE_NAME, Fire_year, remove = F) %>% 
  unite(Fire_year_month, FIRE_NAME, Fire_year, Start_month, remove = F)

write_csv(calfire_concat, path = "calfire_concat.csv")
